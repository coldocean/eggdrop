...
#    - added !cancelquote command, which allows администратор to cancel the
#      last recorded quote.
...
#######################
  ...
  variable allowed_chans {#lobby #wunderbar #help}
...
####################################################################
  ...
  proc uninstall {args} {
		putlog "\002Public Quotes System\002 — освобождение ресурсов..."
...
proc pubqsys::quote {nick host hand chan arg} {
  ...
  if {[pubqsys::channel_check $chan] == 0} {
    ...
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Синтаксис — [code bold $chan]!quote[code bold $chan] [code 14 $chan]<[code endcolor $chan]номер цитаты[code 14 $chan]> \[[code endcolor $chan]#chan[code 14 $chan]\][code endcolor $chan]"
    return
  } elseif {([string first # [join [lindex $arg 1]]] == 0) && ([pubqsys::channel_check [lindex $arg 1]] == 0)} {
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Цитаты не включены на [join [lindex $arg 1]]."
    return
  } else {
    if {[lindex $arg 1] == ""} {
    ...
    if { $quotenumber > $numquotes } {
      puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Цитата #$quotenumber ещё не существует. (в базе $numquotes цитат)."
      return
    }   
  ...
  if {[set index1 [lindex $quote 0]] == $quotenumber} {
    ...
    if {[string match *(admin)* [set whodel [join [split [lindex $quote 1]]]]] == 1} { set whodel "администратор" }
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Цитата #$quotenumber удалена [lindex $quote 2] в [lindex $quote 3], удалил(а) $whodel на [join [split [lindex $quote 4]]]."
  } else {
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Несоответствие базы данных."
  }
  return
}
...
proc pubqsys::addquote {nick host hand chan arg} {
  if {[pubqsys::channel_check $chan] == 0} {
    ...
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Синтаксис — [code bold $chan]!addquote[code bold $chan] [code 14 $chan]<[code endcolor $chan]цитата[code 14 $chan]>[code endcolor $chan]"
  } elseif {([string length $arg] < 10) || ([llength [split $arg]] < 3)} {
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Нельзя сохранять цитаты короче 10 символов и 3 слов."
  } else {
    set channame [string replace $chan 0 0]
    variable quoteslist
    ...
    puthelp "PRIVMSG $chan :Цитата #[expr $numquotes + 1] добавлена."
  }
  return
}
...
proc pubqsys::quoteinfo {nick host hand chan arg} {
  if {[pubqsys::channel_check $chan] == 0} {
    ...
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Синтаксис — [code bold $chan]!quoteinfo[code bold $chan] [code 14 $chan]<[code endcolor $chan]номер цитаты[code 14 $chan]> \[[code endcolor $chan]#chan[code 14 $chan]\][code endcolor $chan]"
    return
  } elseif {([string first # [join [lindex $arg 1]]] == 0) && ([pubqsys::channel_check [lindex $arg 1]] == 0)} {
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Цитаты не включены на [join [lindex $arg 1]]."
    return
  ...
  if {[set index1 [lindex $quote 0]] == $quotenumber} {
    puthelp "PRIVMSG $chan :Цитата #$quotenumber сохранена на [join [split [lindex $quote 3]]] [lindex $quote 1] в [lindex $quote 2], автор [join [split [lindex $quote 4]]]."
  } elseif {$index1 == "-deleted-"} {
    if {[string match *(admin)* [set whodel [join [split [lindex $quote 1]]]]] == 1} { set whodel "администратор" }
    puthelp "PRIVMSG $chan :Цитата #$quotenumber удалена [lindex $quote 2] в [lindex $quote 3], удалил(а) $whodel на [join [split [lindex $quote 4]]]."
  } else {
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Несоответствие базы данных."
  }
  return
...
proc pubqsys::delquote {nick host hand chan arg} {
  if {[pubqsys::channel_check $chan] == 0} {
    ...
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Синтаксис — [code bold $chan]!delquote[code bold $chan] [code 14 $chan]<[code endcolor $chan]номер цитаты[code 14 $chan]>[code endcolor $chan]  (вы должны быть автором цитаты)"
    return
  } else {
    set channame [string replace $chan 0 0]
    set quotenumber [string trimleft $arg "0"]
  ...
  if {[set index1 [lindex [split $quote] 0]] == $quotenumber} {
    if {[set author [join [lindex [split $quote] 4]]] != $nick} {
      puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Вы не можете удалить эту цитату. Её автор — [code bold $chan]$author[code bold $chan]."
      return
    }
    ...
    pubqsys::write_quotes $channame
    puthelp "PRIVMSG $chan :Цитата #$quotenumber удалена."
  } elseif {$index1 == "-deleted-"} {
    if {[string match *(admin)* [set whodel [join [split [lindex $quote 1]]]]] == 1} { set whodel "администратор" }
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Цитата #$quotenumber уже была удалена [lindex $quote 2] в [lindex $quote 3], удалил(а) $whodel на [join [split [lindex $quote 4]]]."
  } else {
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Несоответствие базы данных."
  }
  return
...
proc pubqsys::randquote {nick host hand chan arg} {
  if {[pubqsys::channel_check $chan] == 0} {
    ...
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Синтаксис — [code bold $chan]!randquote[code bold $chan] [code 14 $chan]\[[code endcolor $chan]#chan[code 14 $chan]\][code endcolor $chan]"
  } elseif {($arg != "") && ([string first # [join $arg]] == 0) && ([pubqsys::channel_check [join $arg]] == 0)} {
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Цитаты не включены на [join $arg]."
    return
  } else {
    if {[lindex $arg 0] == ""} {
      set channame [string replace $chan 0 0]
    ...
    if {$numquotes == 0} {
      puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Действительных цитат не найдено."
      return
    }
...
proc pubqsys::lastquote {nick host hand chan arg} {
  if {[pubqsys::channel_check $chan] == 0} {
    ...
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Синтаксис — [code bold $chan]!lastquote[code bold $chan] [code 14 $chan]\[[code endcolor $chan]#chan[code 14 $chan]\][code endcolor $chan]"
  } elseif {($arg != "") && ([string first # [join $arg]] == 0) && ([pubqsys::channel_check [join $arg]] == 0)} {
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Цитаты не включены на [join $arg]."
    return
  } else {
    ...
    if {[lindex $lastrawquote 0] == "-deleted-"} {
      if {[string match *(admin)* [set whodel [join [split [lindex $lastrawquote 1]]]]] == 1} { set whodel "администратор" }
      puthelp "PRIVMSG $chan :[code 14 $chan]Последняя цитата (#$numquotes) не существует — удалена [lindex $lastrawquote 2] в [lindex $lastrawquote 3], удалил(а) $whodel на [join [split [lindex $lastrawquote 4]]]. Ищу последнюю действительную цитату...[code endcolor $chan]"
    }
...
proc pubqsys::findquote {nick host hand chan {arg}} {
  if {[pubqsys::channel_check $chan] == 0} {
    ...
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Синтаксис — [code bold $chan]!findquote[code bold $chan] [code 14 $chan]\[[code endcolor $chan]-all[code 14 $chan]/[code endcolor $chan]#chan[code 14 $chan]\] <[code endcolor $chan]аргументы поиска[code 14 $chan]>  [code 07 $chan]|[code endcolor $chan] Используйте кавычки для точного совпадения. [code underline $chan]Примеры[code underline $chan] : [code bold $chan]!findquote a test[code bold $chan] (найдёт цитаты, содержащие букву [code bold $chan]a[code bold $chan] [code underline $chan]и[code underline $chan] и слово [code bold $chan]test[code bold $chan]).  [code bold $chan]!findquote \"a test\"[code bold $chan] (найдёт цитаты с точным совпадением \"[code bold $chan]a test[code bold $chan]\").[code endcolor $chan]"
    return
  } elseif {([string first # [join $arg]] == 0) && ([lindex $arg 1] != "") && ([pubqsys::channel_check [join [lindex $arg 0]]] == 0)} {
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Цитаты не включены на [join [lindex $arg 0]]."
    return
    ...
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Аргумент поиска должен быть не короче 3 символов."
    return
  } else {
    if {([string first # [join $arg]] == 0) && ([lindex $arg 1] != "")} {
      ...
      if {[llength $pubqsys::allowed_chans] == 1} {
        puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Примечание[code underline $chan] : Опция -all излишня: цитаты включены только на одном канале."
        set channame [string replace $chan 0 0]
      } else {
        set channame "-all"
      }
    ...
    if {$channame != "-all"} {
      ...
      foreach channameall $pubqsys::allowed_chans {
        ...
        if {[file exists "data/quotes.db/quotes.$channameall"]} {
          variable quotesfile [open "data/quotes.db/quotes.$channameall" r]
          set quoteslist(-all) "$quoteslist(-all) [split [read -nonewline $quotesfile] \n]"
          close $quotesfile
        }
      }
      if {$quoteslist(-all) == ""} {
        puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : База данных не найдена."
        return
      }
    }
    ...
    if {$numresults == 0} {
      puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] Поиск не дал результатов."
      return
    } elseif {$numresults == "1"} {
      if {$channame != "-all"} {
        puthelp "PRIVMSG $chan :[code underline $chan]$numresults цитат(ы) найдено[code underline $chan] :"
      } else {
        puthelp "PRIVMSG $chan :[code underline $chan]$numresults цитата найдена на [lindex [split [join $filter_quotes]] 3][code underline $chan] :"
      }
    } elseif {($numresults > $pubqsys::maxfindquotetotal) && ($pubqsys::maxfindquotetotal != 0)} {
      puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] Поиск вернул более $pubqsys::maxfindquotetotal результатов (точно $numresults). Уточните аргументы поиска, чтобы сократить число результатов."
      return
    } else {
      if {$channame != "-all"} {
        set foundquotenumbers "[code underline $chan]$numresults цитат(ы) найдено[code underline $chan] : "
        ...
        puthelp "PRIVMSG $chan :[code underline $chan]$numresults цитат(ы) найдено на следующих каналах[code underline $chan]: $resultchanlist"
        puthelp "PRIVMSG $chan :[code underline $chan]Недавние цитаты[code underline $chan]:"
...
proc pubqsys::deletedquoteinfo {nick host hand chan arg} {
  if {[pubqsys::channel_check $chan] == 0} {
    ...
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Синтаксис — [code bold $chan]!deletedquoteinfo[code bold $chan] [code 14 $chan]<[code endcolor $chan]номер цитаты[code 14 $chan]>[code endcolor $chan]"
    return
  } else {
    set channame [string replace $chan 0 0]
    set quotenumber [string trimleft $arg "0"]
  ...
  if {[set index1 [lindex $quote 0]] == $quotenumber} {
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Цитата #$quotenumber существует — информации об удалении нет."
  } elseif {$index1 == "-deleted-"} {
    if {[lindex $quote 6] == ""} {
      puthelp "PRIVMSG $chan :Информация об этой цитате недоступна."
    } else {
      puthelp "NOTICE $nick :Цитата #$quotenumber удалена [lindex $quote 2] в [lindex $quote 3], удалил(а) [join [split [lindex $quote 1]]] ([join [split [lindex $quote 6]]]) на [join [split [lindex $quote 4]]]. Цитату добавил(а) [join [split [lindex $quote 14]]] [join [split [lindex $quote 13]]] [lindex $quote 11] в [lindex $quote 12]. Вот цитата:"
...
proc pubqsys::undelquote {nick host hand chan arg} {
  if {[pubqsys::channel_check $chan] == 0} {
    ...
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Синтаксис — [code bold $chan]!undelquote[code bold $chan] [code 14 $chan]<[code endcolor $chan]номер цитаты[code 14 $chan]>[code endcolor $chan]"
    return
  } else {
    set channame [string replace $chan 0 0]
    set quotenumber [string trimleft $arg "0"]
  ...
  if {[set index1 [lindex [split $quote] 0]] == $quotenumber} {
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Цитата #$quotenumber существует — восстанавливать не нужно."
  } elseif {$index1 == "-deleted-"} {
    ...
    pubqsys::write_quotes $channame
    puthelp "PRIVMSG $chan :Цитата #$quotenumber восстановлена."
  } else {
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Несоответствие базы данных."
  }
  return
...
proc pubqsys::forcedelquote {nick host hand chan arg} {
  if {[pubqsys::channel_check $chan] == 0} {
    ...
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Синтаксис — [code bold $chan]!forcedelquote[code bold $chan] [code 14 $chan]<[code endcolor $chan]номер цитаты[code 14 $chan]>[code endcolor $chan]"
    return
  } else {
    set channame [string replace $chan 0 0]
    set quotenumber [string trimleft $arg "0"]
...
proc pubqsys::cancelquote {nick host hand chan arg} {
  if {[pubqsys::channel_check $chan] == 0} {
    ...
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Синтаксис — [code bold $chan]!cancelquote[code bold $chan] [code 14 $chan]<[code endcolor $chan]номер последней цитаты[code 14 $chan]>[code endcolor $chan]"
    return
  } else {
    set channame [string replace $chan 0 0]
    variable quoteslist
    ...
    set numquotes [llength $quoteslist($channame)]
    if { $arg != $numquotes } {
      puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Указанный номер не совпадает с номером последней цитаты. Чтобы снизить риск случайного удаления, укажите номер именно последней цитаты."
      return  
    }
  ...
  if {([lindex $quote 0] == $numquotes) || (([lindex $quote 0] == "-deleted-") && ([lindex $quote 10] == $numquotes))} {
    ...
    pubqsys::write_quotes $channame
    puthelp "PRIVMSG $chan :Последняя сохранённая цитата отменена."
...
proc pubqsys::read_quotes {chan channame command} {
  variable quoteslist
  if {(![file exists "data/quotes.db/quotes.$channame"]) && ($command != "addquote") && ($command != "findquoteall")} {
    if {[file exists "data/quotes.db/quotes.$channame.bak"]} {
      file copy "data/quotes.db/quotes.$channame.bak" "data/quotes.db/quotes.$channame"
      puthelp "PRIVMSG $chan :[code 04 $chan]::: [code underline $chan]Критическая ошибка[code underline $chan][code endcolor $chan] : База данных для #$channame не найдена. Найдена свежая резервная копия и автоматически восстановлена."
      putlog "[code 04 $chan]\[Public Quotes System] [code underline $chan]Критическая ошибка[code underline $chan][code endcolor $chan] : База данных для #$channame не найдена. Найдена свежая резервная копия и автоматически восстановлена."
    } else {
      puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : База данных для #$channame не найдена."
      return "missing"
    }
  }
  if {[file exists "data/quotes.db/quotes.$channame"] != 0} {
    set quotesfile [open "data/quotes.db/quotes.$channame" r]
    ...
    if {([llength $quoteslist($channame)] == 0) && ($command != "addquote") && ($command != "findquoteall")} { 
      puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Пустая база данных."
      return "missing"
    }
  } else {
...
proc pubqsys::write_quotes {channame} {
  variable quoteslist
  if {![file exists "data/quotes.db"]} { file mkdir "data/quotes.db" }
  set quotesfile [open "data/quotes.db/quotes.$channame" w]
...
proc pubqsys::isargclean {chan data} {
  if {[regexp {[\{\}\\]} $data] != 0} {
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Нельзя использовать следующие символы в аргументах поиска команды !findquote : [code bold $chan]\{ \} \\[code bold $chan]"
    return "dirty"
  } elseif {[expr ([set is_even [regsub -all {\"} $data "" isargcleantmp]] / 2) * 2] != $is_even} {
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Нечётное число кавычек в аргументах поиска. Кавычки задают точное совпадение и должны использоваться парами. [code underline $chan]Пример[code underline $chan] : [code bold $chan]!findquote \"a test\"[code bold $chan]"
    return "dirty"
  } elseif {[regexp {""} $data] != 0} {
    puthelp "PRIVMSG $chan :[code 04 $chan]:::[code endcolor $chan] [code underline $chan]Ошибка[code underline $chan] : Вы использовали [code bold $chan]\"\"[code bold $chan] в аргументах поиска — это пустой запрос точного совпадения, команда [code bold $chan]!findquote[code bold $chan] его не допускает. Добавьте текст между кавычками. [code underline $chan]Пример[code underline $chan] : [code bold $chan]!findquote \"a test\"[code bold $chan]"
    return "dirty"
  } else {
    return "clean"
  }
...
proc pubqsys::antiflood {chan type} {
  ...
  if { $instance($chan$type) >= $max_instances } {
    if { $antiflood_msg($chan$type) == 0 } {
      ...
      if {$type != "global"} {
        putquick "privmsg $chan :[code 04 $chan]:::[code 14 $chan] Обнаружен флуд по [code bold $chan]!$type[code bold $chan] : максимум $max_instances запросов за $instance_length сек.[code endcolor $chan]"
      } else {
        putquick "privmsg $chan :[code 04 $chan]:::[code 14 $chan] Обнаружен флуд в системе цитат : максимум $max_instances команд за $instance_length сек.[code endcolor $chan]"
      }
...
proc pubqsys::backup_db {min hour day month year} {
  putlog "\00314\[Public Quotes System\]\003 Резервное копирование баз данных..."
  foreach channame $pubqsys::allowed_chans {
    set channame [string replace $channame 0 0]
    file copy -force "data/quotes.db/quotes.$channame" "data/quotes.db/quotes.$channame.bak"
  }
}
...
putlog "\002*Public Quotes System v$pubqsys::version*\002 by MenzAgitat (\037\00312http://www.boulets-roxx.com\003\037) загружен"
