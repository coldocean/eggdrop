...
if {[set ttmp(temp) [info tclversion]]<"8.4"} {
  putlog "Tcl Version Conflict. This Script Requires Tcl 8.4 or Higher."
  putlog "[info hostname] is currently running old Tcl ver. $ttmp(temp) (PatchLevel: [info patchlevel])"
  putlog "Contact your shell to have them update the tcl library, or go to mytclscripts.com for a script that will run on old tcl."
...
if {[info exists t2(ver)]} {
  ...
  } else {  die "BogusTrivia script update: Bot must be restarted!"  }
}
...
proc TAddHEvent {els} {  global t2 botnick
 ...
 if {$els ne ""} {  puts $thifile "::X: Extra History:"
...
proc TReadHist {num {opt -1} } {  global t2 tclr botnick
 ...
 if {$find ne "1+"} {  set lut [lindex $utls 0]  ;  set plls [lindex $plls 0]
   ...
   if {$num=="1"} {  set sholine "$tclr(-d10) YESTERDAYS Top 10 - "
     ...
     set sholine "$tclr(-m10) ${month}S Top 10 - "
     if {[llength $plls]>"20"} {  set sholin2 "$tclr(-m10) ${month}S Top 11-20: - "  }
   }
   set cnt 0
   ...
   if {$dwm=="D"} {  set sholine "$tclr(-d10) Top Player of: - "
   } elseif {$dwm=="W"} {  set sholine "$tclr(-w10) Top Player of: - "
   } else {  set sholine "$tclr(-m10) Top Player of: - "  }
   set cnt 0
   foreach lut $utls player $plls {  incr cnt
    ...
    if {$dwm=="D"} {
      ...
      if {$cnt=="1"} {  append sholine "$clr2 \002Last Week:\002 $clr3 $nk $pt "
      } else {  append sholine "$clr2 \002$cnt Weeks Ago:\002 $clr3 $nk $pt "  }
    } else {
...
set t2(-ads) "$t2(script) v$t2(ver) by $t2(auth)"
